...
- Evidence cites code, docs or commits with the project prefix. Where a claim rests only on the engineering notes of the projects and no code or commit shows it, the entry says **(notes only)**.
...
This file complements the chapters: the chapters say what to do, this file says why and what else was on the table. The method behind the library decisions is the three-phase method (Nabizada, Drath, Fay; at Automatisierungstechnik, 2026, forthcoming), with earlier steps at EKA 2026 and ETFA 2026; its analysis items A1 to A4 and patterns P1 to P8 are summarised in [10-new-language-recipe.md](10-new-language-recipe.md). Terms are in [glossary.md](glossary.md).
...
- **Evidence:** file and line, or commit sha, with project prefix. Starter files are cited as `starter/...`.
- **Status.**
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:233-239` (pending only on a real edit), `:632-647` (Update), `:685-700` (Refresh asks first); `starter/plugin/Aml.Editor.Plugin.Efl/EflPlugin.xaml.cs:1-14`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj:53-62`; `AMLFPB.js: Aml.Editor.Plugin.FPB/Aml.Editor.Plugin.FPB.csproj:50-53`; `AMLPetriNet: dotnet/PtMapper.Web/Program.cs:1-3`.
- **Status:** in force.

### D-03 One repository and one solution

- **Context:** AMLFPB.js builds only when the plugin, the mapper, FPB.JS and OCL.NET sit side by side in a fixed folder layout. CI was broken for months because a new project reference was not mirrored in the workflow, and the flat checkout pointed a relative path outside the workspace.
...
- **Evidence:** `AMLPetriNet: .github/workflows/ci.yml:15-17`; `AMLFPB.js: .github/workflows/build.yml:26-48`; `AMLFPB.js` commit `d93f319` ("replicate local repo layout, check out OCL.NET").
...
- **Options:** (a) rule classes in the plugin, only the OCL files in the mapper (AMLFPB.js); (b) validator and report in the mapper (AMLPetriNet, starter).
- **Decision:** (b).
...
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/Validation/Vdi3682Validator.cs:38`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbMapper.Conversion.csproj:13-21` (OCL rules as embedded resources); `AMLPetriNet: dotnet/PtMapper.Web/Program.cs:217`.
- **Status:** differs between projects; (b) recommended.
...
- **Evidence:** `AMLPetriNet: README.md:37`; `AMLPetriNet: web/package.json:21`; `AMLPetriNet: web/src/index.js:9-45`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: README.md:7-8` names AMLFPB.js as the FPD counterpart.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:521-529`; `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs:49-56`; `AMLFPB.js` commit `238391d`.
- **Status:** differs between projects; (b) recommended.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:685-700`; `AMLFPB.js: Aml.Editor.Plugin.FPB/Views/IhView.xaml.cs:47-48`, `:261-267`, `:686-703`; `AMLFPB.js` commit `7188bd7`.
- **Status:** differs between projects; (b) recommended.
...
- **Evidence:** `AMLPetriNet: README.md:6`; `starter/dotnet/Efl.Conversion/EflJson.cs:7-40`.
- **Status:** in force.
...
- **Decision:** Node `<position>` is the centre; arcs carry only intermediate bend points, the docking points are recomputed; node label offsets are relative to the node centre; an arc label offset is measured from the middle of the polyline's middle segment, copied from the modeler's own `getWaypointsMid`. The arc label position travels end to end (decisions E02 and E03 of the September 2026 audit).
...
- **Evidence:** `AMLPetriNet: web/src/pnml/index.js:21-33`, `:177-185`.
- **Status:** in force.
...
- **Decision:** (c). The model carries `Warnings` and `Unresolved`; the plugin shows reading warnings in the findings list (audit decision E20); the updater never deletes an unresolved element (D-48).
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/Models/PtModels.cs:79-92`; `AMLPetriNet: dotnet/PtMapper.Conversion/CaexToPtNet.cs:124-133`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:745-748`; `starter/dotnet/Efl.Conversion/EflJson.cs:33-34`; `starter/dotnet/Efl.Conversion/Models.cs:28-34` (`Unresolved`).
- **Status:** in force.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/EflJson.cs:156-203`; `starter/web/src/io/json.js:3-13`, `:108-141`, `:161`.
- **Status:** in force.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/EflJson.cs:45-49`, `:74-78`; `starter/web/src/io/json.js:15-16`, `:27-32`; tests `AFileFromANewerFormatVersionIsReadWithAWarning` and `AFileWithoutAFormatVersionIsReadAsVersionOne` in `starter/dotnet/Efl.Tests/ExampleAndGeometryTests.cs:39`, `:49`; browser check in `starter/web/tools/verify-modeler.mjs:37-42`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNames.cs:57-62`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Tests/GoldenTests.cs:34-38` (the v1 quirk).
- **Status:** proposed.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNames.cs:3-10`, `:87-95`; `starter/dotnet/Efl.Conversion/EflLibraries.cs:46-58`, `:60-80`; `fpb-aml-mapper` commit `5b4c4bf` (VDI_ prefix).
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLibraries.cs:197-199`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper` commits `5f6e629` (CreateClassInstance), `0af45f2` (AML base inheritance, dual SupportedRoleClass); `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:84-95`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper` commit `157d1c9`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/DiagramInterchangeLibrary.cs:5-17`; `AMLPetriNet: dotnet/PtMapper.Conversion/DiagramInterchangeLibrary.cs:12-15`; `AMLFPB.js` commit `a59b5ea`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Conversion/MapperOptions.cs:21-43`, `:81-86`; commit `157d1c9`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Conversion/CaexToFpbJson.cs:50-53`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpdLibraries.cs:95-97`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Tests/ReferenceSurvivalTests.cs:8-15`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNames.cs:64-84`.
- **Status:** in force.
...
- **Decision:** (b) (audit decision E09). The ObjectReferences file ships as an embedded resource because the plugin has no library folder in reach; OMG_DD is built in code. The library artefact itself keeps references.
- **Consequences:** Class paths inside the own libraries lose the alias prefix when a library is embedded, and the writer must follow whichever form a document uses (D-26). AMLFPB.js still writes an ExternalReference for OMG_DD. The starter embeds OMG_DD from the published file rather than building it in code (D-90).
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLibraries.cs:28-58`, `:59-97`; `AMLPetriNet: dotnet/PtMapper.Conversion/ObjectReferencesLibraryFile.cs:7-22`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtSelfContained.cs:5-16`.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLibraryLoan.cs:6-19`, `:79-83`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtDiPaths.cs:7-23`.
- **Status:** in force.
...
- **Decision:** Reference by file name, never by a URL containing a token or credentials. fpb-aml-mapper switched to the relative file name, and the starter uses the file name `AutomationML_Base_Libraries_AMLEd2_2.11.0.aml`. AMLPetriNet still uses a public URL that carries a share token in its user-info part, the same shape the FPD mapper removed (known issue).
- **Consequences:** A relative name depends on the reading tool's search path; a URL depends on the host staying up. Do not put credentials or tokens into a path that is written into documents.
- **Evidence:** `fpb-aml-mapper` commit `e866fad`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbMappings.cs:124-131`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtNames.cs:98-104`; `starter/dotnet/Efl.Conversion/EflLibraries.cs:28-34`.
- **Status:** differs between projects; the file name recommended, a token in AMLPetriNet's path is a known issue.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/Efl.Conversion.csproj:13-18`; `starter/dotnet/Efl.Conversion/EflDiagramInterchange.cs:29-99`; `starter/dotnet/Efl.Conversion/EflLibraries.cs:40-58`, `:106-113`; `starter/dotnet/Efl.Tool/Program.cs:97-109`; `starter/dotnet/Efl.Tests/DiagramInterchangeTests.cs:41-128`.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLibraries.cs:14-17`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:1046-1049`; `starter/dotnet/Efl.Conversion/EflNames.cs:83-105`.
- **Status:** in force.
...
- **Decision:** Interface ids on write. AMLPetriNet does not split at a colon at all, because an id may contain one (audit decision E16). The FPD reader and the starter still accept the `ElementId:InterfaceId` form from externally authored files and take the part after the last colon.
- **Consequences:** Pick one policy and keep it in one helper.
- **Evidence:** `fpb-aml-mapper` commit `2426e61`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtCaex.cs:24-33`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/CaexToFpbJson.cs:923-934`; `starter/dotnet/Efl.Conversion/EflWrite.cs:191-205`; `starter/dotnet/Efl.Conversion/CaexToEfl.cs:167-179`.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/EflUpdater.cs:23-24`, `:65`, `:177-184`; `starter/plugin/Aml.Editor.Plugin.Efl/Diagnostics/PluginSettings.cs:31-34`; `starter/dotnet/Efl.Web/Program.cs:63-69`.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:108-113`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:192-196`; `starter/dotnet/Efl.Conversion/EflToCaex.cs:187-206`; `starter/dotnet/Efl.Conversion/EflUpdater.cs:97-107`; test `AFlowFromANodeToItselfGetsTwoPortsAndSurvivesTheRoundTrip` in `starter/dotnet/Efl.Tests/RoundTripTests.cs:190-214`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/CaexToPtNet.cs:59-63`, `:79-83`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs:7-16`, `:22-47`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:554-565`; `fpb-aml-mapper` commit `860c3ec` (FPB.JS ids as AML ids).
- **Status:** differs between projects; (b) recommended.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs:51-62`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs:72-100`, `:119-139`.
- **Status:** in force.
...
- **Decision:** Normalise for display; valid names stay untouched; ASCII only, with German letters transliterated and other diacritics stripped (audit decision E11); the writer does not overwrite a stored id that differs only by normalisation.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIdText.cs:5-18`, `:24-57`, `:99-105`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:214-217`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:849-870`.
- **Status:** in force.
...
- **Decision:** The first element keeps its id, later ones get a number, a note says which (audit decision E12). An earlier branch that deleted copies was dead code and removed.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIdText.cs:161-212`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:159-163`.
- **Status:** in force.
...
- **Decision:** AMLPetriNet compares exactly through one `StringComparer` (audit decision E13); a reference differing only in case does not resolve and its arc is kept as unresolved. The id space that allocates new ids stays case-insensitive, so no new id differs from an existing one by case only. fpb-aml-mapper strips braces and compares case-insensitively.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtCaex.cs:12-22`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs:129`.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:544-565`, `:198`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/CaexToFpbJson.cs:328-339`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper` commits `99efcda`, `7e8cc4b`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:1225-1228`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs:66-70`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:437-458`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:22-36`; `fpb-aml-mapper` commit `05384f7`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:105-116`; `starter/dotnet/Efl.Conversion/EflUpdater.cs:14-21`.
- **Status:** in force.
...
- **Decision:** Preconditions first (duplicate ids, connection ends, carried classes), loan closed in `finally`, class lookup tolerant of duplicates (audit decision E14). Error messages end with "Nothing was written".
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:56-114`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:70-76`; `starter/dotnet/Efl.Conversion/EflUpdater.cs:37-40`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:16-44`.
- **Status:** in force.
...
- **Decision:** An arc arriving without a name keeps its stored name (audit decision E17); a place or transition name the user cleared stays empty (E18, together with D-60).
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtWrite.cs:188-197`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:7-20`, `:198-203`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:658-660`; `starter/dotnet/Efl.Conversion/CaexToEfl.cs:106-116`; `starter/dotnet/Efl.Conversion/EflUpdater.cs:67-69`, `:109-121`, `:157-164`, `:174-175`, `:275-294`; test `AnUpdateKeepsAFlowTheCanvasCouldNotShow` in `starter/dotnet/Efl.Tests/RoundTripTests.cs:165`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs:217-223`; commit `4a083b6`.
- **Status:** in force.
...
- **Decision:** Id falls back to the CAEX name, claimed in the same order as the reader; the label falls back to the CAEX name when the id is empty (audit decision E15).
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/CaexToPtNet.cs:205-214`, `:237-253`.
- **Status:** in force.
...
- **Decision:** Lazily.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs:121-133`.
...
- **Decision:** P/T nets: arrange missing positions at import and before display, tell the user how many were placed. FPD: layout is stored; a layout-free document still opens with fallbacks (D-54).
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLayout.cs:5-20`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:872-874`.
- **Status:** in force.
...
- **Options:** longest-path relaxation on the raw graph (superseded); breadth-first columns (used briefly after the audit, notes only); DFS cycle breaking, longest path on the acyclic rest, barycentre ordering with neighbour swaps scored by crossings plus arcs through nodes, back edges routed around the drawing.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtLayout.cs:70-85`, `:179-187`, `:490-500`, `:560-573`, `:615-622`.
- **Status:** in force; earlier variants superseded.

### D-54 A layout-free FPD document still gets connection visuals

- **Context:** The FPB.JS importer dereferences a connection's visual entry unconditionally; a layout-free AML aborted the whole import and left an empty canvas.
- **Options:** fix the importer; have the mapper always emit a visual with at least two waypoints.
- **Decision:** The mapper emits a straight centre-to-centre line when nothing is stored. Fallback points carry the same `original` substructure as port-derived points, or the echo cycle drifts. The importer's own null check is still missing.
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Conversion/CaexToFpbJson.cs:492-501`; commit `2c80523`.
- **Status:** in force.
...
- **Decision:** The latter. The round-trip comparison counts `PortCoordinate` values as geometry (audit decision E04).
- **Consequences:** A new shape needs the renderer outline and the C# geometry changed together.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Conversion/PtRoundTripCheck.cs:261`; `starter/dotnet/Efl.Conversion/EflGeometry.cs:3-9`, `:29-37`.
- **Status:** in force.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/EflLayout.cs:175-289`, `:335-372`; `starter/dotnet/Efl.Tests/LayoutTests.cs:6-13`, `:85-197`; `starter/web/tools/screenshot.mjs:1-20`.
...
- **Evidence:** `AMLPetriNet: web/build.mjs:24-43`; `starter/web/build.mjs:30`.
- **Status:** in force.
...
- **Evidence:** `starter/web/src/modeling/ElementFactory.js:14-16`.
- **Status:** in force (starter).
...
- **Decision:** Random ids.
- **Evidence:** `starter/web/src/modeling/ElementFactory.js:5-12`.
- **Status:** in force (starter).
...
- **Evidence:** `AMLPetriNet: web/src/bridge.js:26-35`, `:124-137`, `:148-157`, `:175-186`, `:193-200`, `:201-212`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: web/src/index.js:20-45`, `:58-67`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:153-158`; `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs:169-180`; `starter/plugin/Aml.Editor.Plugin.Efl/EflPlugin.xaml.cs:93-97`.
- **Status:** in force.

### D-62 Echo detection by content, not by a time window

- **Context:** AMLFPB.js ignored changes for 3 s after each push. The window swallowed genuine quick edits; it was stamped before buffered pushes and opened fake windows; the page's `importing` flag reset in `requestAnimationFrame`, which WebView2 pauses in hidden tabs, and stuck.
...
- **Decision:** Content baseline. AMLFPB.js keeps the timed window only as a fallback for pages that send no baseline.
...
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/Views/IhView.xaml.cs:47-52`, `:204-222`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:233-239`; `starter/plugin/Aml.Editor.Plugin.Efl/EflPlugin.xaml.cs:148-173`; `starter/web/src/bridge.js:111-127`; `AMLFPB.js` commit `238391d`.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:124-127`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Bridge/PtWebView.cs:131-148`; `AMLFPB.js: Aml.Editor.Plugin.FPB/Bridge/FpbWebView.cs:101-113`.
- **Status:** in force.
...
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs:289-293`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:288-296`.
- **Status:** in force.
...
- **Decision:** `OriginID` plus `FileName` (AMLPetriNet, starter); two new unsaved documents with an empty `FileName` count as distinct. Two same-named files in different folders still compare equal; the full path is the recommended next step and untested.
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:298-344`; `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs:386-398`; `starter/plugin/Aml.Editor.Plugin.Efl/EflPlugin.xaml.cs:220-237`.
...
- **Decision:** In the view; `IToolBarIntegration` removed from both plugins.
- **Evidence:** `AMLFPB.js` commits `cb4dc50`, `a96fc36`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:117-122`; `starter/plugin/Aml.Editor.Plugin.Efl/EflPlugin.xaml:20-25`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet` commit `529b8c9`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Diagnostics/PluginSettings.cs:15-20`, `:30`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:1117-1146`; `AMLFPB.js: Aml.Editor.Plugin.FPB/Diagnostics/PluginSettings.cs:18`; `AMLFPB.js: Aml.Editor.Plugin.FPB/Views/IhView.xaml.cs:423-430`.
...
- **Decision:** Names, automatic validation; double click selects on the canvas by the canvas id.
- **Evidence:** `AMLPetriNet` commit `529b8c9`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:732-748`; `AMLFPB.js` commit `43c3751` (host selects an element in the viewer).
- **Status:** in force.
...
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs:39-56`; commit `238391d`.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Bridge/EditorSaver.cs:1-11`.
...
- **Decision:** Exact pins; never ship the contract. The starter's package tests fail when the contract or Aml.Engine is in the package.
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/Aml.Editor.Plugin.FPB.csproj:24-29`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj:26-44`; `AMLPetriNet: README.md:81-83`; `starter/plugin/Aml.Editor.Plugin.Efl.Tests/PackageTests.cs:48-61`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj:45-62`, `:82-87`, `:104-110`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj:16-20`.
...
- **Evidence:** `AMLFPB.js: Aml.Editor.Plugin.FPB/Aml.Editor.Plugin.FPB.csproj:126-140`; `AMLFPB.js: Aml.Editor.Plugin.FPB/fpbjs-assets/index.html:90-98`; `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj:90-102`.
...
- **Evidence:** `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs:89-91`; `starter/plugin/Aml.Editor.Plugin.Efl.Tests/PackageTests.cs:80-95`.
- **Status:** in force.
...
- **Evidence:** `tools/new-language.mjs:1-24`, `:94-126`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Web/Program.cs:1-6`; `fpb-aml-mapper: server.js:101-102`.
...
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Web/Program.cs:8-13`, `:187`; `fpb-aml-mapper: dotnet/FpbMapper.Web/Program.cs:24`, `:56`; `starter/dotnet/Efl.Web/Program.cs:83`.
- **Status:** in force.
...
- **Decision:** The latter.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Web/Program.cs:56`, `:68`, `:158`.
- **Status:** in force.
...
- **Decision:** One helper.
- **Evidence:** `starter/dotnet/Efl.Conversion/EflDocuments.cs:10-18`.
- **Status:** in force.
...
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Tests/TestCollection.cs:1-7`; `AMLPetriNet: dotnet/PtMapper.Tests/TestCollection.cs:1-7`; `fpb-aml-mapper` commit `05384f7`.
- **Status:** in force.
...
- **Decision:** Whole documents against a second load; an unchanged sync must change no line; an "everything else" aspect fails on any change outside the diagram (audit decision E19).
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Tests/PaperRoundTripTests.cs:9-24`; `AMLPetriNet: docs/roundtrip-validation.md:35`, `:122-125`; `AMLPetriNet: dotnet/PtMapper.Conversion/PtRoundTripCheck.cs:170`.
- **Status:** in force.
...
- **Decision:** The latter.
- **Evidence:** `fpb-aml-mapper: dotnet/FpbMapper.Tests/GoldenTests.cs:9-17`, `:28-29`; commit `209bde8`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: web/tools/verify-bridge.mjs:30-43`; `AMLPetriNet: web/tools/verify-paper-session.mjs:1-18`.
- **Status:** in force.
...
- **Evidence:** `AMLPetriNet: web/tools/validate-pnml.mjs:19-20`, `:70`; `AMLPetriNet: docs/pnml-conformance.md:11-17`.
- **Status:** in force.

### D-86 Machine-local fixtures skip; showcase documents are built in the test
...
- **Decision:** `[SkippableTheory]`/`[SkippableFact]` for local files; showcase documents built in the test.
- **Evidence:** `AMLPetriNet: dotnet/PtMapper.Tests/ForeignDocumentSweepTests.cs:30`; `AMLPetriNet: dotnet/PtMapper.Tests/PaperRoundTripTests.cs:36-39`; `fpb-aml-mapper` commit `0e1b7a5`.
- **Status:** in force.
...
- **Evidence:** `starter/dotnet/Efl.Conversion/EflToCaex.cs:32-50`; `starter/dotnet/Efl.Tool/Program.cs:4-14`, `:46-51`, `:84`, `:101`, `:107`, `:116-120`; `starter/.github/workflows/ci.yml:45-55`; `starter/.gitattributes:1-2`; `.github/workflows/playbook.yml:40-50`.
- **Status:** in force (starter).
...
- [ ] Keep evidence citable: a line, a test name or a commit sha; mark anything else as notes only.

## Where to look

| Area | Most telling evidence |
|---|---|
| Update in place, preconditions, ownership | `AMLPetriNet: dotnet/PtMapper.Conversion/PtNetUpdater.cs`, `PtWrite.cs` |
| Ids | `AMLPetriNet: dotnet/PtMapper.Conversion/PtIds.cs`, `PtIdText.cs`, `PtCaex.cs` |
| Libraries in documents | `AMLPetriNet: dotnet/PtMapper.Conversion/PtLibraries.cs`, `PtLibraryLoan.cs`, `PtDiPaths.cs` |
| Reference typing, boundary states | `fpb-aml-mapper: dotnet/FpbMapper.Conversion/MapperOptions.cs`, `FpbJsonToCaex.cs`, `CaexToFpbJson.cs` |
| Layout | `AMLPetriNet: dotnet/PtMapper.Conversion/PtLayout.cs` |
| Plugin lifecycle and echo detection | `AMLPetriNet: Aml.Editor.Plugin.PetriNet/PetriNetPlugin.xaml.cs`; `AMLFPB.js: Aml.Editor.Plugin.FPB/FpbPlugin.xaml.cs`, `Views/IhView.xaml.cs` |
| Modeler patches and bridge | `AMLPetriNet: web/src/index.js`, `web/src/bridge.js`, `web/build.mjs` |
| Packaging | `AMLPetriNet: Aml.Editor.Plugin.PetriNet/Aml.Editor.Plugin.PetriNet.csproj`; `starter/plugin/Aml.Editor.Plugin.Efl.Tests/PackageTests.cs` |
| Starter-only choices | `starter/README.md`, `starter/dotnet/Efl.Conversion/EflLibraries.cs`, `starter/dotnet/Efl.Conversion/EflDiagramInterchange.cs`, `starter/web/src/modeling/ElementFactory.js`, `tools/new-language.mjs` |
| Commit history | AMLFPB.js `238391d`, `a96fc36`; fpb-aml-mapper `157d1c9`, `f5b508e`, `2c80523`, `209bde8`; AMLPetriNet `529b8c9` |
