...
Count-based assertions are a weak first step. AMLFPB.js started with `Roundtrip_PreservesElementCounts` (AMLFPB.js: `Aml.Editor.Plugin.FPB.Tests/CaexToFpbJsonTests.cs`); the mapper later lost characteristic values while counts stayed equal, which needed a dedicated test (fpb-aml-mapper: `dotnet/FpbMapper.Tests/ConversionTests.cs`, `Roundtrip_PreservesCharacteristicSetpointAndType`). Counts also cannot see names, and a golden written from defective output pins the defect instead of catching it. fpb-aml-mapper's `Convert` wrote the name of a sub-process SystemLimit from the process name, which for a sub-process is the name of the decomposed operator, and its round-trip golden recorded `Erhitzen` for a SystemLimit that the input names `SL_Erhitzen`. The defect was invisible on the top-level process, because its process name is taken from its SystemLimit. The mapper now writes the SystemLimit's own name and falls back to the process name only when it has none (fpb-aml-mapper: `dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs`, `BuildProcess`, the `SetIdentification` call for the SystemLimit and its comment), the golden records `SL_Erhitzen` (`dotnet/FpbMapper.Tests/TestData/Temperieren.roundtrip.golden.json`), and a dedicated test compares the SystemLimit name per process after `Convert` and after `UpdateInPlace`, with a guard that the fixture really has a sub-process whose SystemLimit is not named after its operator (fpb-aml-mapper: `dotnet/FpbMapper.Tests/SystemLimitNameTests.cs`, `TheFixtureHasASubProcessWhoseSystemLimitIsNotNamedAfterItsOperator`, `Convert_KeepsTheSystemLimitNamesOfSubProcesses`, `UpdateInPlace_AddingASubProcessKeepsItsSystemLimitName`). Pick fixtures where every name is distinct from every other name, or such coincidences hide bugs.
...
Use real UI interaction only where the UI itself is under test (palette entries, context pad entries, dialogs). Find palette buttons by accessible name (`getByRole('button', { name })`), as FPB.JS does (FPB.JS: `tests/e2e/pages/ModelerPage.js`, `clickPaletteAndPlace`, the `getByRole` call and the comment above it). Later diagram-js 15 releases, such as the 15.26.0 its lock file pins (FPB.JS: `package-lock.json`, entry `node_modules/diagram-js`), set `aria-label` and `role="button"` on palette entries instead of `title`, so a lookup by title finds nothing.
...
- **Documents from other tools.** Load every real document you have, check that it reads, that a sync without edits changes nothing, that syncing twice settles, that the model read after sync equals the model read before, and that files with the vocabulary but no model (libraries, other languages) are not mistaken for one (AMLPetriNet: `dotnet/PtMapper.Tests/ForeignDocumentSweepTests.cs`, from `ItReadsAsANetTheModelerCouldShow` to `AFileWithoutANetIsNotMistakenForOne`). Alias-qualified class paths must read like unaliased ones (fpb-aml-mapper: `dotnet/FpbMapper.Tests/AliasToleranceTests.cs`, `Convert_AliasQualifiedDocument_MatchesUnaliasedResult`). Layout-free input must still convert completely and open in the modeler. fpb-aml-mapper emits only the layout the document stores, and FPB.JS arranges what lacks layout on import (FPB.JS: `app/fpb/importer/JSONImporter.js`, the `needsLayout` and `layoutImportData` calls). The mapper tests pin both halves: with stored layout every element and connection has a visual entry, without connection layout the connections get none while shapes keep theirs, and without any layout all data still arrives with no visual entry at all (fpb-aml-mapper: `dotnet/FpbMapper.Tests/LayoutFreeConversionTests.cs`, summary of `LayoutFreeConversionTests`, `Convert_WithStoredLayout_EveryElementAndConnectionHasAVisual`, `Convert_WithoutConnectionLayout_ConnectionsGetNoVisual_ShapesKeepTheirs`, `Convert_WithoutAnyLayout_EmitsNoVisualInformationButAllData`). Invented geometry in the mapper would keep the modeler from arranging those connections itself (see [Layout](08-layout.md)).
...
- **A release workflow without tests.** FPB.JS publishes to npm on a tag after `npm ci` and `npm run build`, with no test step (FPB.JS: `.github/workflows/release.yml`, job `publish`). A separate workflow runs the unit tests, the Playwright end-to-end tests and the build on every push and pull request (FPB.JS: `.github/workflows/test.yml`, jobs `unit`, `e2e` and `build`), but the release does not wait for it, so a tag on a commit whose tests fail still publishes (see [Pitfalls](09-pitfalls.md), PF-CI-02 and PF-CI-03). Gate publishing on the tests: run them in the release workflow, or make the publish job depend on them.
...
| Browser round trip, interop, bridge, labels, tokens, numbers, SVG | AMLPetriNet: web/tools/roundtrip.mjs, web/tools/verify-interop.mjs, web/tools/verify-bridge.mjs, web/tools/verify-labels.mjs, web/tools/verify-tokens.mjs, web/tools/verify-numbers.mjs, web/tools/verify-svg.mjs |
...
| Connection visuals for layout-free documents | fpb-aml-mapper: dotnet/FpbMapper.Tests/LayoutFreeConversionTests.cs |
| Names that a count cannot see | fpb-aml-mapper: dotnet/FpbMapper.Tests/SystemLimitNameTests.cs |
| Mapper CI | fpb-aml-mapper: .github/workflows/test.yml |
| Modeler CI and release | FPB.JS: .github/workflows/test.yml, .github/workflows/release.yml |
