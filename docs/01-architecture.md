...
Auto layout of missing geometry belongs to exactly one side. AMLPetriNet puts it in the mapper, as in the table. The FPD tool chain puts it in the modeler: FPB.JS arranges whatever lacks visual information before its importer builds shapes (`FPB.JS: app/fpb/importer/JSONImporter.js`, `needsLayout` and `layoutImportData` in the `IMPORT_EVENTS.IMPORT_REQUEST` handler; `FPB.JS: app/fpb/layout/AutoLayout.js`), and fpb-aml-mapper passes missing layout through: no visual entry for a link without stored points, no `ViewInformation` for an element without visual data, the SystemLimit included (`fpb-aml-mapper: dotnet/FpbMapper.Conversion/CaexToFpbJson.cs`, `ParseProcess`; `fpb-aml-mapper: dotnet/FpbMapper.Conversion/FpbJsonToCaex.cs`, comments in `AddElement` and `BuildProcess`; pinned by `fpb-aml-mapper: dotnet/FpbMapper.Tests/LayoutFreeConversionTests.cs`, `Convert_WithoutAnyLayout_EmitsNoVisualInformationButAllData`). The mapper's comment names the reason: synthesised straight lines would stop the modeler from arranging. Pick one side and let the other pass missing layout through untouched; the mapper is the better place when a CLI or web app reads the same documents, because the FPD web API hands out JSON without layout ([08 Layout](08-layout.md) §3).
...
| Missing layout | arranged in the mapper (`PtLayout`) | arranged in FPB.JS on import (`AutoLayout`), passed through by the mapper | one place only; the mapper when several consumers read the documents |
